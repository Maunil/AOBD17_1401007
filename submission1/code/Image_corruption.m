%This code is written by dlaptev 
%Here i have just modified it to work based on my requirements 

%read image and add the mask
img = double(imread('original_image.jpg'))/255;
msk = double(imread('mask.png'))/255;
img = imresize(img, 0.125);
msk = imresize(msk, 0.260);
img = rgb2gray(img);
msk = rgb2gray(msk);
img = img(50:50+255, 100:100+255);
% imshow(img);
% msk = zeros(size(img));
msk = msk(:,1:1:256);
img_corrupted = img;
...
% applying Robust PCA
lambda = 0.032258064; % lemda = 1/sqrt(max(dim))
tic
...
% show the results
figure;
imshow(img_corrupted), title('Corrupted image')
figure;
imshow(img_final), title('Recovered image')
figure;
imshow(img_reconstructed), title('Recovered low-rank')
figure;
imshow(img_noise), title('Recovered sparse')
